...
from lib.modelarts_tools import push_data_back_on_modelarts
...
class DCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        self._writer = SummaryWriter(os.path.join( self._log_dir,'runs'))
        ...
        if self._epoch_num > 0 and 'load_model_dir' in self._train_kwargs:
            self.load_model(self._train_kwargs.get('load_model_dir'))
    ...
    def load_model(self,load_model_dir):
        self._setup_graph()
        model_path = os.path.join(load_model_dir, 'epo%d.tar' % self._epoch_num)
        assert os.path.exists(model_path), 'Weights at epoch {} not found from {}'.format(self._epoch_num, model_path)
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            self._logger.info('tensorboard recorded :{},mean_loss:{},batches_seen:{}'.format('{} loss'.format(dataset), mean_loss, batches_seen))
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            # print('writer {}:{}:{} '.format('training loss',
            #                         np.mean(losses),
            #                         batches_seen))
            self._logger.info(
                'tensorboard recorded :{},mean_loss:{},batches_seen:{}'.format('training loss', np.mean(losses),
                                                                               batches_seen))
            ...
            self._logger.info('pushed data back at epoch: %d' % epoch_num)
